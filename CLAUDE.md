...
- **`rule-engine.ts`** — Automation rule engine. `evaluateCondition()` evaluates nested boolean condition trees against store state. `RuleEngine` class handles edge-triggered rules (fire once on transition) and continuous rules (lever tracks condition result). Includes cooldown tracking, action execution (lever switching, notifications, group enable/disable), and duration-based rule evaluation.
